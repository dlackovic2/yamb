 ...
 */
export async function syncDiceRoll(gameId, playerId, diceValues, diceLocked, rollsRemaining, options = {}) {
  try {
    ...
    */
    ...
    console.error(`Sync dice ${normalizedAction} error:`, error);
    throw error;
  }
}
 ...
 * @returns {Promise<void>}
 */
export async function syncScoreEntry(gameId, playerId, scorecard, category, column, value, options = {}) {
  try {
    ...
    if (stateError) {
      if (supportsPendingAnnouncements && column === 'announce' && isMissingColumnError(stateError)) {
        console.warn('Pending announcement column missing during score sync; falling back to legacy behaviour.');
