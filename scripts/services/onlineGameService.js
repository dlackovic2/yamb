  ...
  try {
    ...
    if (gameError) {
      ...
      if (gameError.code === '23502' && gameError.message.includes('host_player_id')) {
        throw new Error('Database setup error: Please make host_player_id nullable in Supabase. See SUPABASE_FIX.md for instructions.');
      }
...
export async function leaveGame(gameId, playerId) {
  try {
    ...
    if (!existingPlayers || existingPlayers.length === 0) {
      console.warn('No player record found while leaving; assuming already removed.', { gameId, playerId });
      return;
    }
